...
void AssemblerMain () {
    ...
    ReadUserInput (user_input, &commands_list, &program_len, &special, &labels, &labels_idx);
    fclose (user_input);
    ...
    //Вывод дополнительной информации о программе (кол-во чисел в машинном коде)
    FILE *program_info = fopen ("program_data/program.info", "w"); //Дополнительная информация о программе
    fprintf (program_info, "%zu\n", program_len + special - labels_idx);
    fprintf (program_info, "//THIS FILE IS AUTOGENERATED//");
    fclose (program_info);
...
size_t GetCommandsNum (FILE *user_input, struct stat file_info, char **input_start) {
    ...
    *input_start = buf + i;
    ...
    // Запрет компиляции программы если после END есть команды
    //bool end_flag = false; //Флаг наличия в программе конца
    ...
    for ( ; ( (i < file_info.st_size) && (buf[i] != EOF) ); ++i) {
        ...
        if ( (i >= 2) && (buf[i - 2] == 'E') && (buf[i - 1] == 'N') && (buf[i] == 'D') ) {
            buf[i + 1] = '\0';
            buf[i + 2] = EOF;
            end_flag = true;
        }
    ...
    /* Запрет компиляции программы если после END есть команды
    //Если не найден конец программы
    if (!end_flag) {
        printf ("No END found\n");
        exit (2);
    }
    ...
    for (int pass = 1; pass <= 2; ++pass) {
        ...
        for (size_t i = 0; i < program_len; ++i) {
            ...
            //Если в строке содержится метка или CALL
                ...
                if (pass == 2) fprintf (user_lst, "%08zu %08x %08x "#name" %d\n", shift, CMD_##name, location_idx, location_idx); }
